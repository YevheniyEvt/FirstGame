...
def drop_tetromino(tetromino, mouse_in_rect):
    ...
    if (mouse_in_rect and
            tetromino.get_start_x()+tetromino.get_size()*tetromino.get_index_width()*0.9 < int(GAME_FIELD_WIDTH / SIZE)*SIZE and
            tetromino.get_start_y() + tetromino.get_size()*tetromino.get_index_height()*0.9 < int(GAME_FIELD_HEIGHT / SIZE)*SIZE):
...
def stick_tetromino_to_square(tetromino):
    ...
    if tetromino.get_start_x() >= 0 and tetromino.get_start_y() >= 0:
        for rectangle in tetromino.get_square():
            rectangle_x = int(rectangle[0]/size)*size*1.03
            rectangle_y = int(rectangle[1]/size)*size*1.03
